...
.btn-submit { width: 100%; padding: 14px 24px; background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; border: none; border-radius: 8px; font-size: 16px; font-weight: 500; font-family: 'Poppins', sans-serif; cursor: pointer; transition: transform .15s, box-shadow .15s; margin-top: 10px; box-shadow: 0 4px 12px rgba(var(--tint),.3); }
.btn-submit:hover { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(var(--tint),.4); }
...
.btn-open { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; border: none; box-shadow: 0 4px 12px rgba(var(--tint),.3); transition: transform .15s, box-shadow .15s; }
.btn-open:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(var(--tint),.4); }
...
.theme-toggle { position: absolute; top: 14px; right: 14px; width: 42px; height: 42px; border-radius: 50%; border: 1px solid rgba(var(--tint),.35); background: rgba(255,255,255,.55); color: var(--text); font-size: 19px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: transform .2s, box-shadow .2s, background .2s; backdrop-filter: blur(8px); box-shadow: 0 4px 14px rgba(0,0,0,.08); z-index: 5; }
.theme-toggle:hover { transform: translateY(-2px) rotate(12deg); box-shadow: 0 8px 22px rgba(var(--tint),.3); }
html.dark { --background: #0b1220; --card: #151e33; --text: #e2e8f0; --muted: #94a3b8; color-scheme: dark; }
html.dark body { background: radial-gradient(900px 700px at 12% -8%, rgba(var(--tint),.14), transparent 60%), var(--background); }
html.dark .header { border-bottom-color: rgba(var(--tint),.4); }
html.dark .form-section { background: rgba(21,30,51,.72); border-color: rgba(var(--tint),.22); box-shadow: 0 10px 34px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.06); }
html.dark .form-section h2 { border-bottom-color: rgba(var(--tint),.25); }
...
html.dark .form-group input:focus, html.dark .form-group select:focus, html.dark .form-group textarea:focus { background: rgba(17,24,44,.95); border-color: var(--primary); box-shadow: 0 0 0 3px rgba(var(--tint),.3); }
...
html.dark .upload-area:hover { background: rgba(var(--tint),.12); border-color: var(--primary); }
...
/* ==== Tema Warna (pilih lewat titik warna di header) ==== */
html[data-color="biru"] { --primary:#0284c7; --primary-light:#7dd3fc; --secondary:#1d4ed8; --tint:2,132,199; --primary-soft:#e0f2fe; }
html[data-color="hijau"] { --primary:#059669; --primary-light:#6ee7b7; --secondary:#0d9488; --tint:5,150,105; --primary-soft:#d1fae5; }
html[data-color="ungu"] { --primary:#7c3aed; --primary-light:#c4b5fd; --secondary:#8b5cf6; --tint:124,58,237; --primary-soft:#ede9fe; }
html[data-color="oranye"] { --primary:#ea580c; --primary-light:#fdba74; --secondary:#c2410c; --tint:234,88,12; --primary-soft:#ffedd5; }
html[data-color="tosca"] { --primary:#0d9488; --primary-light:#5eead4; --secondary:#0f766e; --tint:13,148,136; --primary-soft:#ccfbf1; }
.color-dots { display: flex; gap: 9px; justify-content: center; align-items: center; margin-top: 14px; flex-wrap: wrap; }
.cdot { width: 20px; height: 20px; border-radius: 50%; border: 2px solid #fff; box-shadow: 0 0 0 1px rgba(0,0,0,.12), 0 2px 6px rgba(0,0,0,.18); cursor: pointer; padding: 0; transition: transform .15s, box-shadow .15s; }
.cdot:hover { transform: scale(1.18); }
.cdot.active { box-shadow: 0 0 0 3px rgba(var(--tint),.4); transform: scale(1.1); }
html.dark .cdot { border-color: rgba(255,255,255,.75); }
...
def build_head(title):
    return ('<!DOCTYPE html>\n<html lang="id">\n<head>\n'
            ...
            '    <script src="doc-capture.js"></script>\n'
            '    <script>(function(){var k="tb_theme",s;try{s=localStorage.getItem(k)}catch(e){}var on=s?s==="dark":(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches);var ap=function(x){document.documentElement.classList.toggle("dark",x);var b=document.getElementById("themeToggle");if(b)b.textContent=x?"☀️":"🌙"};ap(on);var ck="tb_color",cv="merah";try{var v=localStorage.getItem(ck);if(v)cv=v}catch(e){}document.documentElement.setAttribute("data-color",cv);var DOTS=[["merah","#dc2626","Merah Tribik"],["biru","#0284c7","Biru Samudra"],["hijau","#059669","Hijau Zamrud"],["ungu","#7c3aed","Ungu Nebula"],["oranye","#ea580c","Oranye Senja"],["tosca","#0d9488","Tosca Segar"]];document.addEventListener("DOMContentLoaded",function(){var b=document.getElementById("themeToggle");if(b){b.addEventListener("click",function(){var x=!document.documentElement.classList.contains("dark");ap(x);try{localStorage.setItem(k,x?"dark":"light")}catch(e){}})}var box=document.getElementById("colorDots");if(box){for(var i=0;i<DOTS.length;i++){(function(d){var bt=document.createElement("button");bt.type="button";bt.className="cdot"+(d[0]===cv?" active":"");bt.style.background=d[1];bt.title=d[2];bt.setAttribute("aria-label","Tema "+d[2]);bt.addEventListener("click",function(){cv=d[0];document.documentElement.setAttribute("data-color",d[0]);try{localStorage.setItem(ck,d[0])}catch(e){}var all=box.querySelectorAll(".cdot");for(var j=0;j<all.length;j++)all[j].classList.remove("active");bt.classList.add("active")});box.appendChild(bt)})(DOTS[i])}}});})();</script>\n'
            '</head>\n<body>\n'
            '    <div class="container">\n'
            '        <div class="header">\n'
            ...
            '            <div class="color-dots" id="colorDots" aria-label="Pilih tema warna"></div>\n'
            '        </div>\n'
            '        <form id="autoForm">\n')
...
COMMON_JS = '''
    <script>
        ...
        /* --- Document Auto Capture (doc-capture.js) — deteksi & auto-capture --- */
        var docCams = {};   // doc -> instance DocumentAutoCapture

        function commitPhoto(doc, dataURL) {
        ...
        function openCamera(doc) {
            ...
            if (!preview || !controls || !video || !overlay) { playSound('error'); return; }
            if (typeof DocumentAutoCapture === 'undefined') {
                Modal.alert('Komponen pemindai (doc-capture.js) belum termuat.', 'Kamera');
                return;
            }
            ...
            cam.start()['catch'](function (err) {
                preview.classList.remove('active'); controls.style.display = 'none';
                Modal.alert('Kamera tidak dapat diakses: ' + (err.message || err), 'Kamera');
            });
            docCams[doc] = cam;
        }
        ...
        function cancelCamera(doc) {
            var cam = docCams[doc];
            if (cam) { cam.stop(); docCams[doc] = null; }
            var preview = document.getElementById('preview-' + doc);
            var controls = document.getElementById('controls-' + doc);
            if (preview) preview.classList.remove('active');
            if (controls) controls.style.display = 'none';
